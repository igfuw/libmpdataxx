  ...
  namespace bcond
  {
    ...
    > : public detail::bcond_common<real_t, halo>
    {
      ...
      void fill_halos_sgs_vctr(arrvec_t<arr_t> &av, const arr_t &b, const rng_t &j, const int offset = 0)
      {
        ...
        using namespace idxperm;
        const auto &a = av[offset + d];
        a(pi<d>(this->rght_edge_sclr + h, j)) = 2 * b(pi<d>(this->rght_edge_sclr, j)) - a(pi<d>(this->rght_edge_sclr - h, j));
      }
      ...
      void fill_halos_sgs_tnsr(arrvec_t<arr_t> &av, const arr_t &u, const arr_t &div, const rng_t &j, const real_t di)
      {
        using namespace idxperm;
        const auto &a = av[d];
        a(pi<d>(this->rght_edge_sclr + h, j)) = 2 * ( ( 3 * u(pi<d>(this->rght_edge_sclr - 1, j))
                                                      - 2 * u(pi<d>(this->rght_edge_sclr, j))
                                                      -     u(pi<d>(this->rght_edge_sclr - 2, j))
                                                      ) / di
                                                    - div(pi<d>(this->rght_edge_sclr + h, j))
                                                    );
      }
    };
  } // namespace bcond
