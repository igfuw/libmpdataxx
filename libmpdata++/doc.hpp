...
/** @mainpage
  ...
  * | \ref test_step_bombel.cpp "bombel"                      | 2      | 3      | resolved       | \ref libmpdataxx::output::gnuplot "gnuplot" | \ref threads.hpp "threads (openmp/boost)"    | 2D Navier-Stokes (adiabatic, constant density): \f$ \\ \partial_t \theta + \nabla \cdot (\vec{u} \theta) = 0 \\ \partial_t u + \nabla \cdot (\vec{u} u) = -\partial_x \frac{p-p_e}{\rho} \\ \partial_t w + \nabla \cdot (\vec{u} w) = -\partial_z \frac{p-p_e}{\rho} + g \frac{\theta - \theta_e}{\theta_e} \f$ | \image html "../../tests/bombel/figure_pc_u_10.svg"|
  *
  *
  *
