...
namespace libmpdataxx
{
  ...
  // TODO: other option would be to define im as a function of halo in mpdata!
	  ...
	  this->bcyr->fill_halos_vctr_alng(GC_corr[1], this->i^1);
          ...
          this->bcyl->fill_halos_vctr_nrml(GC_corr[0], this->i^h); // TODO: one xchng?
          this->bcyr->fill_halos_vctr_nrml(GC_corr[0], this->i^h);
          this->bcxl->fill_halos_vctr_nrml(GC_corr[1], this->j^h); // TODO: one xchng?
          this->bcxr->fill_halos_vctr_nrml(GC_corr[1], this->j^h);
	  this->mem->barrier();
	  ...
	  this->GC_mono[1]( this->i, jm+h ) = formulae::mpdata::GC_mono<ct_params_t::opts, 1>(psi, this->psi_min, this->psi_max, GC_corr, G, jm, this->i);
          ...
          //std::cerr << "GC[0] MONO" << std::endl;
          //std::cerr << this->GC_mono[0] << std::endl;
          //std::cerr << "GC[1] MONO" << std::endl;
          //std::cerr << this->GC_mono[1] << std::endl;
