	...
	arrvec_t<typename parent_t::arr_t> p_err, lap_p_err;
	  ...
	  lap_p_err[0](this->ijk) = this->lap(p_err[0], this->i, this->j, this->di, this->dj);
        ...
        void pressure_solver_loop_body()
        {
          ...
          {
            tmp_den[v] = this->mem->sum(lap_p_err[v], lap_p_err[v], this->i, this->j);
            if (tmp_den[v] != 0) beta = - this->mem->sum(this->err, lap_p_err[v], this->i, this->j) / tmp_den[v];
            ...
            {
              if (tmp_den[l] != 0) 
                alpha[l] = - this->mem->sum(lap_err, lap_p_err[l], this->i, this->j) / tmp_den[l];
            }
