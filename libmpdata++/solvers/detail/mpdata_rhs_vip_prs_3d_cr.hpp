...
namespace libmpdataxx
{
  namespace solvers
  {
	...
	typename parent_t::arr_t lap_err, p_err, lap_p_err;
	  ...
	  lap_p_err(this->ijk) = this->lap(p_err, this->i, this->j, this->k, this->di, this->dj, this->dk);
        }

        void pressure_solver_loop_body()
        {
          tmp_den = this->mem->sum(lap_p_err, lap_p_err, this->i, this->j, this->k);
          if (tmp_den != 0) beta = - this->mem->sum(this->err, lap_p_err, this->i, this->j, this->k) / tmp_den;
          this->Phi(this->ijk) += beta * p_err(this->ijk);
          this->err(this->ijk) += beta * lap_p_err(this->ijk);

          this->lap_err(this->ijk) = this->lap(this->err, this->i, this->j, this->k, this->di, this->dj, this->dk);

          if (tmp_den != 0) alpha = - this->mem->sum(this->lap_err, lap_p_err, this->i, this->j, this->k) / tmp_den;
