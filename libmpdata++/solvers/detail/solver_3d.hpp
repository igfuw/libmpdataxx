  ...
  namespace solvers
  {
	...
	void xchng(int e) final
	{
	  ...
	  const idx_t<3> &range_ijk
        ...
        {
          this->mem->GC[0](rng_t(i.first(), i.last()-1)^h, j, k) *= dt_ratio;
          this->mem->GC[1](i, rng_t(j.first(), j.last()-1)^h, k) *= dt_ratio;
          this->mem->GC[2](i, j, rng_t(k.first(), k.last()-1)^h) *= dt_ratio;
