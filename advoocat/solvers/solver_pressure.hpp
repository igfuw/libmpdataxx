...
namespace advoocat
{
  namespace solvers
  {
	...
	parent_t(mem, i, j, p),
	Prs_amb(p.Prs_amb),
	err(mem.tmp[std::string(__FILE__)][0][0]),
	...
	tmp_e1(mem.tmp[std::string(__FILE__)][0][6]),
	tmp_e2(mem.tmp[std::string(__FILE__)][0][7]),
        lap_err(mem.tmp[std::string(__FILE__)][0][8]),
	im(i.first() - 1, i.last()),
	jm(j.first() - 1, j.last())
      {}
